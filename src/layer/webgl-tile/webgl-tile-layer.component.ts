...
export class NolWebGLTileLayerComponent implements NolPrefixedOptions<Options>, OnInit, OnChanges, OnDestroy {

  @Input() nolStyle?: Style;
  @Input() nolStyleVariables?: Record<string, number>;
  ...
  @Input() nolSource?: SourceType;
  @Input() nolSources?: SourceType[] | ((extent: Extent, resolution: number) => Array<SourceType>);
  @Input() nolMap?: Map;
  ...
  @Output() nolSourceChange = new EventEmitter<SourceType>();
  ...
  private readonly host = injectLayerHost('nol-webgl-tile-layer');
  ...
  ngOnInit(): void {
    this.instance = new WebGLTileLayer({
      style: this.nolStyle,
    ...
    if (this.nolStyleVariables) {
      this.instance.updateStyleVariables(this.nolStyleVariables);
    }
    ...
    fromEvent<ObjectEvent>(this.instance, 'change:source')
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => {
        this.nolSource = this.instance.getSource() as SourceType;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (nolStyleVariables) {
      this.instance.updateStyleVariables(nolStyleVariables.currentValue);
    }
  }
...
export function useWebGLTileLayer(): NolWebGLTileLayerComponent;
export function useWebGLTileLayer(options: InjectOptions & {optional?: false}): NolWebGLTileLayerComponent;
export function useWebGLTileLayer(options: InjectOptions): NolWebGLTileLayerComponent | null;
export function useWebGLTileLayer(options?: InjectOptions): NolWebGLTileLayerComponent | null  {
  return inject(NolWebGLTileLayerComponent, options || {}) || null;
}
